...
def fig2bytes(fig):
    buf = io.BytesIO()
    fig.savefig(buf, format="jpeg", dpi=300)  # Save as JPEG with 300 DPI resolution
    buf.seek(0)
    return buf.getvalue()


# Download button for map CSV or IMG file
def generate_map_file(file_type, resolution):
    if file_type == "img":
...
# Callback function to handle download request
def handle_download(file_type, resolution):
    map_data = generate_map_file(file_type, resolution)
    if map_data is not None:
        # Initiate download of map file
        if file_type == "img":
            file_extension = "jpg"  # Change file extension to "jpg" for JPEG format
        else:
            file_extension = file_type
        st.download_button(
            label=f"Download Map {file_type.upper()}",
            data=map_data,
            file_name=f"map_data.{file_extension}",
            mime=f"image/{file_type}" if file_type == "img" else "text/csv"
        )

# Add download button for map CSV or IMG file
st.markdown("</br>", unsafe_allow_html=True)
file_type = st.selectbox("Select file type", ["csv", "img"])
resolution = st.slider("Select resolution", 100, 1000, 300)
if st.button("Download Map"):
    handle_download(file_type, resolution)
